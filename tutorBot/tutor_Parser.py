...
def is_valid(inp):                                          # this function takes in an input which is the string that bot receives from user and
                                                            # validates if they used a code block or not
    valid = False
    
    if len(inp) == 0:
        return False
    
    elif inp[0:3] == "```" and inp[len(inp) - 3:len(inp)]:
        return True
        
    return valid                                            # Output: Boolean value, True if they used the codeblock which is the correct format. False 
                                                            # if they did not use the correct Format. 
